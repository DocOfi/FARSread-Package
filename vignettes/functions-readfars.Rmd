...
```{r loadpackFARS}
library(FARSread) ###load package FARSread so we can use the fars_read function
```
...
We have obtained the URL for the 2013, 2014, and 2015 National FARS data from the [NHSTA's](www-fars.nhtsa.dot.gov/Main/index.aspx) website and we will use it to download the file from the NHSTA's ftp site. 

First we'll create folders or directories to contain the zip file.  

```{r createfolders, echo=TRUE, eval=FALSE}
if(!file.exists("data")){ ### create folder for 2013 zip file
  		dir.create("farsdata2013")
}

if(!file.exists("data")){ ### create folder for 2014 zip file
  		dir.create("farsdata2014")
}

if(!file.exists("data")){ ### create folder for 2015 zip file
  		dir.create("farsdata2014")
}
```
...
write.csv(acc_2013, ### We use acc_2013 as the first argument in the function write.csv
          file = "accident2013.csv.bz2", ### create accident_2013.csv.bz2 in the working directory
...
download.file(fileurl2014, destfile="./farsdata2014/FARS2014NationalCSV.zip") ### download file and save in farsdata2014
...
write.csv(acc_2014, ### We use acc_2014 as the first argument in the function write.csv
          file = "accident2014.csv.bz2", ### create accident_2014.csv.bz2 in the working directory
          row.names = FALSE) ### do not create a new column of rownames when you read in the file in R
...
write.csv(acc_2015, ### We use acc_2015 as the first argument in the function write.csv
          file = "accident2015.csv.bz2", ### create accident_2015.csv.bz2 in the working directory
          row.names = FALSE) ### do not create a new column of rownames when you read in the file in R
